...
RabbitMQ кластер (quorum queues: jobs.process, jobs.validate, jobs.dlq)
  ...
  │
  └─ статус задачи пишется в PostgreSQL (через Job state service)

Job state service — следит за жизненным циклом задачи (jobs, job_attempts),
делает retry, возвращает "зависшие" задачи в очередь.

Логи — Promtail читает stdout всех контейнеров, Loki хранит, Grafana
показывает (pull-модель, см. docs/adr/0003-logging-pull-not-push.md).
Метрики — тот же принцип: Prometheus сам ходит в /metrics каждого сервиса.
```
...
| `job-state-service` | State machine задачи, retry, идемпотентность, обработка таймаутов | PostgreSQL (`jobs`, `job_attempts`) | k8s Deployment |
