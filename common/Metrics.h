...
//
// Раньше был header-only и дублировался в каждом сервисе (как HttpServer/
// HttpTypes) — теперь общий, в common/, каждый сервис берёт его через
// target_link_libraries(... simensnx-common) в своём CMakeLists.txt.
// QtAmqpConsumer НЕ вынесен по той же схеме — его копии в job-state-service
// и nx-worker-stub успели разойтись по API (не просто дрейф копипасты), и
// объединение требует отдельного проектирования общего интерфейса.
class Metrics {
public:
